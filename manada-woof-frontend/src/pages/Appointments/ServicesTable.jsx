...
export default function ServicesTable({
  ...
  const handleServicioChange = (e) => {
    ...
    if (idServicio) {
      const s = serviciosArray.find((x) => String(x.id) === idServicio);
      if (s) {
        setRow((prev) => ({
          ...prev,
          valor_servicio: s.precio?.toString() || "",
          duracion_min: (s.duracionMinutos || 0).toString(),
        }));
      }
    }
  };
  ...
  function addItem() {
    ...
    if (!row.id_personal) {
      alert("Asigne responsable");
      return;
    }
    const servicioObj = serviciosArray.find(
      (s) => String(s.id) === String(row.id_servicio)
    );
    const nombre = servicioObj?.nombre ?? "";
    const serviceObj = {
      id: Date.now(),
  ...
  function removeItem(id) {
    setDetalleServicios((prev) => prev.filter((p) => p.id !== id));
  }
      ...
      <table className="table table-sm">
        ...
        <tbody>
          {detalleServicios.length === 0 && (
            <tr>
              <td colSpan="7">No hay servicios agregados</td>
            </tr>
          )}
          {detalleServicios.map((it) => {
            const resp = Array.isArray(personal)
              ? personal.find((p) => Number(p.id) === Number(it.id_personal))
              : null;
            const subtotal =
              (Number(it.valor_servicio) || 0) * (Number(it.cantidad) || 1);
            return (
              <tr key={it.id}>
